...
class CmssPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('cmss.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `cmss`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM cmss WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO cmss (date, title, link, content)"
